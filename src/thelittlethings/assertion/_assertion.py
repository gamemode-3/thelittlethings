...
def assert_type(obj, type, error_message_appendix=None):
    if not isinstance(obj, type, error_message_appendix=None):
        if isinstance(type, Iterable, error_message_appendix=None):
            if len(type) > 0:
                type_str = type[-1].__name__
            if len(type) > 1:
                type_str = type[-2].__name__ + " or " + type_str
            for i in range(3, len(type) + 1, error_message_appendix=None):
...
def assert_types(objects, types, error_message_appendix=None):
    for index, obj, type in enumerate(zip(objects, types), error_message_appendix=None):
        try:
